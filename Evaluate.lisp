    ...
    (if (car res)
        (if (atom e)
            ...
            (case (car e)
              ...
              ((if) (if (evaluate (nth 1 e) env) (evaluate (nth 2 e) env) (evaluate (nth 3 e) env)))
              ((let) (progn (loop for item in (second e) do
                                  (setf env (update-env (car item) (evaluate (car (cdr item)) env) env)))
                            (let ((result (evaluate (first (cdr (cdr e))) env)))
                              (loop for item in (cdr (cdr e)) do
                                    (setf result (evaluate item env)))
                              (values result))
                            ))
              (t (invoke (evaluate (car e) env) (evlist (cdr e) env)))))
        (format t "Invalid number of args: expected ~d, got ~d in ~S~%" (caddr res) (cadr res) e))))

(defun update-env (v1 v2 env)
  (let ((g (cons v1 v2)))
    (if (eq env nil) (return-from update-env (list g)) (return-from update-env (append env (list g))))))
